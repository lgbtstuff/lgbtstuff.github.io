...
function searchOrganizations( catType, searchFor )
{
	...
	for ( var i = 3; i < organizationByLine.length; i += 5 )
		itemList.push( new organization( organizationByLine[ i - 3 ], organizationByLine[ i - 2 ], organizationByLine[ i - 1 ], parseInt( organizationByLine[ i ].substring( 0 ) ), parseInt( organizationByLine[ i ].substring( 5 ) ) ) );
	...
	else if ( catType == 'date' )
	{
		...
		for ( var i = itemList.length - 1; i >= 0; i-- )
		{
			...
			if ( ( itemList[ i ].startDate >= parseInt( searchFor.substring( 0 ) )
				&& itemList[ i ].startDate <= parseInt( searchFor.substring( 7 ) ) )
