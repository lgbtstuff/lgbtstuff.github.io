...
function searchOrganizations( catType, searchFor )
{
	...
	for ( var i = 3; i < organizationByLine.length; i += 5 )
		itemList.push( new organization( organizationByLine[ i - 3 ], organizationByLine[ i - 2 ], organizationByLine[ i - 1 ], Integer.parseInt( organizationByLine[ i ].substring( 0, organizationByLine.indexOf( " " ) ) ), Integer.parseInt( organizationByLine[ i ].substring( organizationByLine.indexOf( " " ) + 1 ) ) ) );
	...
	else if ( catType == 'date' )
	{
		...
		for ( var i = itemList.length - 1; i >= 0; i-- )
		{
			...
			if ( ( itemList[ i ].startDate > Integer.parseInt( searchFor.substring( 0, searchFor.indexOf( "-" ) - 1 ) )
				&& itemList[ i ].startDate < Integer.parseInt( searchFor.substring( searchFor.indexOf( "-" ) + 2 ) ) )
				|| ( itemList[ i ].endDate > Integer.parseInt( searchFor.substring( 0, searchFor.indexOf( "-" - 1 ) ) )
				&& itemList[ i ].endDate < Integer.parseInt( searchFor.substring( searchFor.indexOf( "-" ) + 2 ) ) )
				|| ( itemList[ i ].startDate < Integer.parseInt( searchFor.substring( 0, searchFor.indexOf( "-" - 1 ) ) )
				&& itemList[ i ].endDate > Integer.parseInt( searchFor.substring( searchFor.indexOf( "-" ) + 2 ) ) ) )
			{
				appendEvent( itemList[ i ] );
			}
